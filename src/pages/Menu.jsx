...
import {
  ...
  ShoppingCart, ArrowLeft, Star, Gift, Camera,
  ExternalLink
} from 'lucide-react';
import QRCode from 'qrcode';

// ---- 妆造类型卡片配置（含 emoji + 渐变色 + 描述） ----
const typeCardConfig = {
  '日常妆 / lo妆': { emoji: '🌸', gradient: 'from-pink-400 to-rose-500', bg: 'from-pink-50 to-rose-50', desc: '约1-1.5小时' },
  'COS展妆':        { emoji: '🎭', gradient: 'from-violet-400 to-purple-500', bg: 'from-violet-50 to-purple-50', desc: '约1.5小时' },
  'COS正片妆':      { emoji: '📸', gradient: 'from-indigo-400 to-blue-500', bg: 'from-indigo-50 to-blue-50', desc: '约1.5-2小时' },
  'COS华改妆':      { emoji: '✨', gradient: 'from-amber-400 to-orange-500', bg: 'from-amber-50 to-orange-50', desc: '约2小时' },
  '新娘妆':          { emoji: '👰', gradient: 'from-rose-300 to-pink-400', bg: 'from-rose-50 to-pink-50', desc: '约2.5小时' },
  '舞台妆':          { emoji: '🎪', gradient: 'from-red-400 to-rose-500', bg: 'from-red-50 to-rose-50', desc: '约1.5小时' },
  '写真妆':          { emoji: '📷', gradient: 'from-teal-400 to-cyan-500', bg: 'from-teal-50 to-cyan-50', desc: '约1.5小时' },
  '主持妆':          { emoji: '🎤', gradient: 'from-sky-400 to-blue-500', bg: 'from-sky-50 to-blue-50', desc: '约1.5小时' },
  '伴娘妆':          { emoji: '💐', gradient: 'from-emerald-400 to-green-500', bg: 'from-emerald-50 to-green-50', desc: '约45分钟' },
  '毕业妆':          { emoji: '🎓', gradient: 'from-blue-400 to-indigo-500', bg: 'from-blue-50 to-indigo-50', desc: '约1小时' },
};
...
function getCardConfig(name) {
  ...
  return { emoji: '💄', gradient: 'from-rose-400 to-rose-600', bg: 'from-rose-50 to-rose-50', desc: '' };
}
...
export default function Menu() {
  ...
  const generateConfirmText = () => {
    ...
    lines.push('', '📍 地铁5号线凌大塘站D口附近');
    return lines.filter(l => l !== '').join('\n');
  };

  const handleCopyConfirm = () => {
    navigator.clipboard.writeText(generateConfirmText()).then(() => {
      ...
      <div className="max-w-lg mx-auto space-y-5">
        ...
        {step === 'menu' && (
          <>
            ...
            <div className="text-center pt-3 pb-2">
              ...
              <h1 className="text-xl font-extrabold text-warm-800 mt-3 tracking-tight">西瓜椰约妆</h1>
                  ...
                  return (
                    <button
                      ...
                      className={`relative overflow-hidden rounded-2xl p-4 text-left transition-all duration-300 active:scale-[0.96] ${
                        selected
                          ? `bg-gradient-to-br ${cfg.bg} border-2 border-rose-300 shadow-lg shadow-rose-200/50 scale-[1.01]`
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        <div className={`text-3xl mb-2 drop-shadow-sm ${selected ? 'scale-110' : ''} transition-transform`}>
                          {cfg.emoji}
                        </div>
                        ...
                        <div className="flex items-baseline gap-1.5 mt-2">
                          <span className={`text-lg font-extrabold ${selected ? 'text-rose-600' : 'text-warm-800'}`}>
                            ¥{mt.defaultPrice}
                          </span>
                      ...
                      {selected && (
                        <div className="absolute top-2.5 right-2.5 w-6 h-6 rounded-full bg-gradient-to-br from-rose-400 to-rose-600 flex items-center justify-center shadow-md">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${
                            checked ? 'border-rose-500 bg-rose-500 scale-105' : 'border-gray-200'
                          }`}>
                            {checked && <Check className="w-3 h-3 text-white" strokeWidth={3} />}
                          </div>
                          <span className="text-sm text-warm-800">{svc.name}</span>
                        </div>
                        <span className={`text-sm font-bold ${checked ? 'text-rose-500' : 'text-warm-800/35'}`}>
            ...
            {selectedType && (
              <div className="bg-white/90 backdrop-blur rounded-2xl border border-white shadow-xl shadow-rose-100/30 p-4 animate-scale-in">
                ...
                <div className="border-t border-rose-100/80 mt-3 pt-3 flex items-end justify-between">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-2xl font-extrabold text-rose-600">¥{totalPrice}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {selectedType && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-rose-500 via-pink-500 to-rose-600 text-white font-bold rounded-2xl shadow-xl shadow-rose-300/40 hover:shadow-2xl hover:shadow-rose-300/50 transition-all active:scale-[0.97] text-base tracking-wide"
              >
          ...
          <>
            ...
            <div className={`bg-gradient-to-br ${typeConfig.bg} rounded-2xl border border-white p-4 shadow-sm`}>
              <div className="flex items-center gap-3">
                ...
                <span className="ml-auto text-xl font-extrabold text-rose-600">¥{totalPrice}</span>
              </div>
            </div>
            ...
            <div className="bg-white/90 backdrop-blur rounded-2xl border border-white shadow-xl shadow-rose-100/30 p-5 space-y-3.5">
              <div>
                ...
                <input required className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition placeholder:text-warm-800/25"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="tel" className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition placeholder:text-warm-800/25"
                ...
                <div>
                  ...
                  <input className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition placeholder:text-warm-800/25"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" required className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition"
                    value={date} onChange={e => setDate(e.target.value)} />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-warm-800/50 uppercase tracking-wide mb-1">时间 *</label>
                  <select required className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition"
              ...
              <div>
                ...
                <textarea rows={2} className="w-full px-4 py-3 rounded-xl bg-rose-50/30 border border-rose-100 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent transition resize-none placeholder:text-warm-800/25"
            ...
            <button onClick={handleCreateOrder} disabled={!customerName.trim()}
              className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-rose-500 via-pink-500 to-rose-600 text-white font-bold rounded-2xl shadow-xl shadow-rose-300/40 hover:shadow-2xl transition-all active:scale-[0.97] disabled:opacity-40 disabled:cursor-not-allowed text-base tracking-wide">
          ...
          <>
            ...
            <div className="bg-white/90 backdrop-blur rounded-2xl border border-white shadow-xl shadow-rose-100/30 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-rose-50 to-transparent">
                <span className="text-sm font-bold text-warm-800">📋 订单确认</span>
                <button onClick={handleCopyConfirm}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-xl font-medium transition-all active:scale-95 ${
                    copied ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600 hover:bg-rose-200'
                  }`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={handleCopyConfirm}
                className={`py-3.5 text-sm font-semibold rounded-2xl transition-all active:scale-[0.97] shadow-lg ${
                  copied ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-gradient-to-r from-rose-500 to-pink-500 text-white shadow-rose-200'
                }`}>
                {copied ? '✅ 复制成功！' : '📋 复制发给化妆师'}
              </button>
            </div>
        ...
        {showQR && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl p-6 shadow-2xl animate-scale-in max-w-sm w-full text-center border border-rose-50">
              <button onClick={() => setShowQR(false)}
                className="absolute top-3 right-3 p-2 rounded-xl hover:bg-rose-50 transition-colors">
                <X className="w-5 h-5 text-warm-800/40" />
              </button>

              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-rose-400 to-rose-600 flex items-center justify-center mx-auto mb-4 shadow-lg">
              ...
              {/* 小程序码占位 — 替换为你的真实太阳码 */}
              <div className="bg-rose-50 rounded-2xl p-8 mb-4 flex items-center justify-center">
                <div className="text-center">
                  <div className="text-5xl mb-2">🍉</div>
                  <p className="text-sm font-bold text-rose-600">西瓜椰约妆</p>
