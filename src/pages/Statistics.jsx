...
export default function Statistics() {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white border border-rose-100 rounded-xl p-3 shadow-lg text-sm">
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-warm-800">📊 收入统计</h2>
        <select value={year} onChange={e => setYear(+e.target.value)}
          className="px-3 py-2 rounded-xl border border-rose-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 transition">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: DollarSign, label: `${year}年总收入`, value: `¥${yearlyTotal.toLocaleString()}`, color: 'from-rose-400 to-rose-600' },
